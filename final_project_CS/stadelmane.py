...
class clickableWindow:
	...
	def computer_turn(self):
		...
		'''checks if diagnol right has 3 in a row and places piece if '''
		for i in range(4):
			for j in range(3):
				self.square1=self.grid[i][j]
				self.square2=self.grid[i+1][j+1]
		...
		print(self.grid)
		'''Check to see if 3 of 4 spots are one color'''
		for i in range(4):
			for j in range(6):
				self.square1=self.grid[i][j]
				self.square2=self.grid[i+1][j]
				self.square3=self.grid[i+2][j]
				self.square4=self.grid[i+3][j]
				if self.square1 == self.square2 and self.square4 == self.square2:
					print("here")
					if self.valid_piece(i+2,j):
						self.draw_computer(i+2,j)
						print("here1")
						return
				if self.square1 == self.square3 and self.square3 == self.square4:
					if self.valid_piece(i+1,j):
						print("here2")
						self.draw_computer(i+1,j)
						return

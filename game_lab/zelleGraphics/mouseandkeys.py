...
class clickableWindow:
    ...
    def update(self):
        ...
        if time.time() - self.last_second>1:
            self.last_second = time.time()
            print(time.time()-self.initial_time)
    ...
    def handleKeyPress(self,key):
        ...
        if key.char == "q":
            self.q=Text(Point(100,50),"close the window to quit")
            self.q.draw(self.window)
        ...
        # "keysym" of whatever key I pressed on the keyboard and seeing what came out.
            ...
            if self.lastkey:
                self.lastkey.undraw()
            t = Text(Point(20,550),key.keysym)
            t.setTextColor(color_rgb(255,0,0))
            t.draw(self.window)
            self.lastkey = t
